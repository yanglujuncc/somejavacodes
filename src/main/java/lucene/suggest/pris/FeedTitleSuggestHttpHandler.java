...
public class FeedTitleSuggestHttpHandler implements HttpHandler{

	
	
	protected static Logger logger = Logger.getLogger(FeedTitleSuggestHttpHandler.class.getName());

	
	
	FeedTitleSuggester feedTitleSuggester;
	
	
	public FeedTitleSuggestHttpHandler(FeedTitleSuggester feedTitleSuggester){
		this.feedTitleSuggester=feedTitleSuggester;
	}
	...
	public void handle(HttpExchange xchg) throws IOException {
		...
		List<FeedTitleSuggestion>	feedTitleSuggestions=feedTitleSuggester.suggest(query, maxResult);
		long cost=System.currentTimeMillis()-begin;
		
	//	MSG returnMSG = new MSG(MSG.SUCCESS, "getResultSuccess,suggest cost:"+cost+"'ms.", feedTitleSuggestions);
		
		JSONObject jsonObject=new JSONObject();
		jsonObject.put("code", "success");
		jsonObject.put("msg", "getResultSuccess,suggest cost:"+cost+"'ms.");
		jsonObject.put("content", feedTitleSuggestions);
	...
	//	DataCacher.setMaxSize(10000);
		...
		HttpContext feedTitleSuggestContext = server.createContext("/FeedTitleSuggest", new MutiTheadHttpHandler(threadNum,maxThreadNum,taskQueueSize,new FeedTitleSuggestHttpHandler(feedTitleSuggester)));
		feedTitleSuggestContext.getFilters().add(new ParameterFilter("utf-8"));
		
		server.setExecutor(null); // creates a default executor
		
		server.start();
		System.out.println("  FeedTitleSuggest Server start ...");
